...
// Helper: Count occurrences without regex to prevent ReDoS
function countOccurrences(text: string, term: string): number {
  ...
  while (true) {
    ...
    if (pos >= 0) {
      count++;
      pos += term.length; // Move past the match
    } else {
      break;
    }
  }
...
export function scoreDocument(doc: SearchDocument, terms: string[]): number {
  ...
  for (const term of terms) {
    ...
    // Replaced RegExp with safer countOccurrences
...
export function searchDocuments(
  ...
  // Also filtering very short terms to reduce noise
