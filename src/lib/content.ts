...
export const getPaper = cache((lang: string, id: string): ParsedContent | null => {
  ...
  for (const paper of papers) {
    if (paper.frontmatter.id === id) return paper;
    if (normalize(paper.frontmatter.id) === requested) return paper;
  }
  return null;
});
...
export const getBooks = cache((lang: string = 'en'): ParsedContent[] => {
  ...
  for (const e of entries) {
    ...
    if (stat.isDirectory()) {
      ...
      // Sort chapters using sortChapterFilenames logic to ensure correct first chapter pick
...
export const getBook = cache((lang: string, id: string): ParsedContent | null => {
  // Use getBooks to leverage cache and avoid file I/O
