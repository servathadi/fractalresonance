...
export function parseFrontmatter(content: string): ParsedContent {
  // Use a non-greedy regex to match only the frontmatter block without capturing the massive body text.
  // This avoids catastrophic memory pressure when parsing large markdown files while fully
  // supporting whitespace and trailing comments on the `---` delimiters.
  const emptyResult = { frontmatter: { title: '', id: '' }, body: content, raw: content };
