...
export function parseFrontmatter(content: string): ParsedContent {
  // Fast-fail: if the content doesn't start with ---, it has no frontmatter
  ...
  // Note: Removed the greedy body capture to significantly improve performance on large files.
  const fmRegex = /^---[^\S\r\n]*(?:#.*)?\r?\n([\s\S]*?)\r?\n---[^\S\r\n]*(?:#.*)?\r?\n?/;
