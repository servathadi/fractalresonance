...
export function parseFrontmatter(content: string): ParsedContent {
  // ⚡ Bolt: Fast path to bypass massive regex allocations for non-frontmatter files
  ...
  // ⚡ Bolt: Omit full file capture ([sS]*)$ which causes immense memory pressure on huge files.
  // Use non-greedy frontmatter extract and `slice` for the remainder.
  const fmRegex = /^---[^\S\r\n]*(?:#.*)?\r?\n([\s\S]*?)\r?\n---[^\S\r\n]*(?:#.*)?\r?\n?/;
