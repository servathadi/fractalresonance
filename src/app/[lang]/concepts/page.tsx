...
import { getLanguages, getConcepts, isCurrentReadingContent, matchesPerspectiveView } from '@/lib/content';
...
export default async function ConceptsPage({ params }: Props) {
  ...
  const concepts = getConcepts(lang).filter(c => isCurrentReadingContent('concept', c.frontmatter) && matchesPerspectiveView(c.frontmatter.perspective, 'kasra'));
