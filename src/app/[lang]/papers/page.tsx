...
import { getLanguages, getPapers, isCurrentReadingContent, matchesPerspectiveView } from '@/lib/content';
...
export default async function PapersPage({ params }: Props) {
  ...
  const papers = getPapers(lang).filter(p => isCurrentReadingContent('paper', p.frontmatter) && matchesPerspectiveView(p.frontmatter.perspective, 'kasra'));
