import { getConcepts, getContentEpistemicStatus, isCurrentReadingContent, matchesPerspectiveView, type PerspectiveView } from '@/lib/content';
...
export function ConceptsIndex({
  ...
  const concepts = getConcepts(lang).filter((c) => isCurrentReadingContent('concept', c.frontmatter) && matchesPerspectiveView(c.frontmatter.perspective, view));
  ...
  const items: ConceptsGridItem[] = concepts.map((c) => {
    ...
    return {
      ...
      epistemicStatus: getContentEpistemicStatus('concept', fm),
    };
  });
