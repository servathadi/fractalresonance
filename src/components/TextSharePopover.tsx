...
export function TextSharePopover() {
  ...
  return (
    <div
      ...
      role="toolbar"
      aria-label="Text share options"
    >
      {/* Copy */}
      <button
        onClick={copyText}
        title={copied ? 'Copied!' : 'Copy text'}
        aria-label={copied ? 'Copied successfully' : 'Copy selected text'}
      >
        {copied ? (
      ...
      {/* Twitter/X */}
      <button onClick={shareTwitter} title="Share on X" aria-label="Share on X">
      ...
      {/* Share link */}
      <button onClick={shareLink} title="Copy page link" aria-label="Copy page link">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
