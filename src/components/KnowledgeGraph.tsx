...
export function KnowledgeGraph({ data, lang }: KnowledgeGraphProps) {
  ...
  const searchIndex = useMemo(() => {
    ...
    const searchable: { node: GraphNode; idLower: string; titleLower: string }[] = [];
    ...
    return { map, searchable };
  }, [filtered.nodes]);

  const suggestions = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (q.length < 2) return [];
    return searchIndex.searchable
      .filter((item) => item.idLower.includes(q) || item.titleLower.includes(q))
      .map(item => item.node)
      .slice(0, 8);
  }, [searchIndex, query]);
          ...
          <input
            ...
            onKeyDown={(e) => {
              ...
              const match =
                ...
                searchIndex.searchable.find(item => item.idLower.includes(q) || item.titleLower.includes(q))?.node ||
                null;
