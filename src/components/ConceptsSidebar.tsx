...
import { getConcepts, isCurrentReadingContent, matchesPerspectiveView, type PerspectiveView } from '@/lib/content';
...
export function ConceptsSidebar({ lang, currentId, basePath, view, variant = 'desktop' }: ConceptsSidebarProps) {
  const concepts = getConcepts(lang)
    .filter((c) => isCurrentReadingContent('concept', c.frontmatter) && (view ? matchesPerspectiveView(c.frontmatter.perspective, view) : true))
