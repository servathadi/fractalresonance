...
1. La constante de couplage dépendante de l'échelle $k^*(\mu)$
2. La topologie de routage et de dissipation d'entropie
...
Où :
- $S$ est l'entropie thermodynamique
- $C$ est la cohérence
- $\mu$ indexe les strates de résolution
...
Une approximation de premier ordre cohérente avec la mise à l'échelle fractale est :
...
2. $\mu_1$ — environnement physique (chaleur)
3. $\mu_2$ — régulation biologique
...
Interprétation du signe de couplage :
- $J_i > 0$ : favorise l'alignement (attractif)
