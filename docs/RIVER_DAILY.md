...
- `RIVER_FRC_TELEGRAM_NOTIFY=1`: send a Telegram message when a draft is created and when a draft is published
  - `RIVER_FRC_TELEGRAM_CHAT_ID=<chat_id>` (group or user id)
  - Requires `RIVER_BOT_TOKEN` or `TELEGRAM_BOT_TOKEN` present in `/home/mumega/mirror/.env`
